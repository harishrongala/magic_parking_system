...
$.post("get_space_details.php",{user:pointer_user,lat:pointer_lat,lng:pointer_lng},function (result){
	result=$.parseJSON(result);
	$('#rmspaceid_modal').html(result[0].space_id);
	$('#rmaddress_modal').html(result[0].address);
	var pic="../image"+result[0].picture;
	$('#parking_space_image').attr('src',pic);
	if(result[0].maxduration=="NA")
	{
		var duration_val="Unlimited";
	}
else{
	var duration_val=result[0].maxduration+" Hrs";
}
...
{
	$.post("remove_parkingspace.php",{user:pointer_user,lat:pointer_lat,lng:pointer_lng},function(data){
...
{
	$.post("retrieve_owner_spaces.php",{userid: pointer_user},function(data){get_markers(data);});
}
